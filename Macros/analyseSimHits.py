...
handleEB  = Handle ('vector<PCaloHit>')
handleEE  = Handle ('vector<PCaloHit>')
...
for iev, event in enumerate(events): 
    ...
    event.getByLabel( ("g4SimHits","EcalHitsEB"), handleEB)
    event.getByLabel( ("g4SimHits","EcalHitsEE"), handleEE)
...
outfile = R.TFile("output_sim_analysis.root", "recreate")
outfile.cd()
for h in hs:
    h.Write()
outfile.Close()
